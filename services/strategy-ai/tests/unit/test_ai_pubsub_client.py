...
import httpx
import pytest
from strategy_ai.clients.pubsub import PubSubError, PubSubPublisher, PubSubSubscriber
...
async def test_publisher_sends_base64_payload_and_returns_message_id() -> None:
    ...
    req = captured[0]
    assert req.url.path == "/v1/projects/trade-ai-dev/topics/strategy-signals-b:publish"
    body = json.loads(req.content.decode())
    msg = body["messages"][0]
    assert base64.b64decode(msg["data"]) == b'{"symbol":"7203"}'
    assert msg["attributes"] == {"source": "AI"}
...
async def test_publisher_raises_on_missing_message_ids() -> None:
    ...
    async with _build_publisher(_handler) as pub:
        with pytest.raises(PubSubError):
            await pub.publish("strategy-signals-b", data=b"x")
...
async def test_subscriber_pull_decodes_messages() -> None:
                ...
                "receivedMessages": [
                    {
                        ...
                        "message": {
                            "messageId": "m1",
                            "data": base64.b64encode(b"feat-1").decode("ascii"),
                    ...
                    },
                    {
                        ...
                        "message": {
                            "messageId": "m2",
                            "data": base64.b64encode(b"feat-2").decode("ascii"),
                        },
                    },
                ]
            },
    ...
    async with _build_subscriber(_handler) as sub:
        msgs = await sub.pull("features-sub", max_messages=10)

    assert len(msgs) == 2
    assert msgs[0].ack_id == "a1"
    assert msgs[0].message_id == "m1"
    assert msgs[0].data == b"feat-1"
    assert msgs[0].attributes == {"symbol": "7203"}
    assert msgs[1].data == b"feat-2"
    assert msgs[1].attributes == {}

    body = json.loads(captured[0].content.decode())
    assert body == {"maxMessages": 10, "returnImmediately": False}
    assert captured[0].url.path == "/v1/projects/trade-ai-dev/subscriptions/features-sub:pull"
...
async def test_subscriber_pull_returns_empty_when_no_messages() -> None:
    ...
    async with _build_subscriber(_handler) as sub:
        msgs = await sub.pull("features-sub", max_messages=5, return_immediately=True)
    assert msgs == []
...
async def test_subscriber_pull_returns_empty_on_read_timeout() -> None:
    ...
    async with _build_subscriber(_handler) as sub:
        msgs = await sub.pull("features-sub", max_messages=5)
    assert msgs == []
...
async def test_subscriber_pull_rejects_non_positive_max_messages() -> None:
    ...
    async with _build_subscriber(_handler) as sub:
        with pytest.raises(ValueError):
            await sub.pull("features-sub", max_messages=0)
...
async def test_subscriber_acknowledge_skips_on_empty_list() -> None:
    ...
    async with _build_subscriber(_handler) as sub:
        await sub.acknowledge("features-sub", [])
    assert called == 0
...
async def test_subscriber_acknowledge_posts_ack_ids() -> None:
    ...
    async with _build_subscriber(_handler) as sub:
        await sub.acknowledge("features-sub", ["a1", "a2"])
...
async def test_subscriber_raises_on_4xx() -> None:
    ...
    async with _build_subscriber(_handler) as sub:
        with pytest.raises(PubSubError):
            await sub.pull("features-sub", max_messages=5)
