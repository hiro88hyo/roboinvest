...
import httpx
import pytest
from strategy_ai.clients.supabase import SupabaseError, SupabaseWriter
...
async def test_insert_strategy_logs_posts_rows_and_returns_count() -> None:
    ...
    async with SupabaseWriter(
        ...
        n = await writer.insert_strategy_logs(
            [
                _signal(symbol="7203", action=Action.BUY),
                _signal(
    ...
    assert body[0]["reasoning"] == "ai pick"
    assert body[0]["created_at"].startswith("2026-04-20T09:00")
    assert body[1]["reasoning"] == "downtrend confirmed"
