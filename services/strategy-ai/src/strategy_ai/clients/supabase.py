...
class SupabaseWriter:
    """Write-only Supabase client. strategy-ai は strategy_logs に AI シグナルを書く。"""
    ...
    async def insert_strategy_logs(self, signals: Iterable[StrategySignal]) -> int:
        ...
        Pub/Sub 再配信で同一 signal_id が来ても重複しないよう
        `on_conflict=signal_id` で upsert する。空入力は no-op。
        """
        rows = [_signal_to_row(s) for s in signals]
        if not rows:
            logger.debug("supabase upsert skipped: table=strategy_logs rows=0")
            return 0
        ...
        logger.debug("supabase upsert: table=strategy_logs rows=%d", len(rows))
        return len(rows)
