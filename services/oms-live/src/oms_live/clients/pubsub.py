"""Pub/Sub (REST) subscriber client for OMS Live.

OMS Live は ``live-orders`` 1 本だけを pull する (raw-market-data は購読しない;
板情報は kabu API から自前で取れるため擬似約定の入力としては不要)。
publish も無し (約定結果は Supabase に書き込み)。

oms-paper の subscriber を踏襲: synchronous Pull + REST、streaming Pull は使わない。
"""
