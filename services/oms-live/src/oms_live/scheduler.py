"""14:50 JST デイクローズアウト用のスケジューラ (oms-live)。

oms-paper の同名モジュールと同型。``run_closeout_scheduler`` は 1 日 1 回
``runner.run_closeout()`` を呼ぶ無限ループ。テストでは ``iterations`` を有限にし、
``clock`` / ``sleep`` を差し替えて時間進行を制御する。

休場日でも cron 自体は発火するが、``run_closeout`` 内の ``trading_style != day``
チェックで no-op になるため、ここでは曜日判定を持たない。
"""
